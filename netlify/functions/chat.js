...
// Shared report-writer functions (2026-07-02): getPrompt, extractWeeklyPlan,
// writeWeeklyPlan, extractQuarterly, writeQuarterlyReview, extractVTPlaybook,
// wrapVTPlaybookInTemplate — also used by generate-report-background.mts, both
// pulling from the SAME file (netlify/functions/lib/report-writers.mjs). Moved
// out of this file to kill the manual-sync-or-drift risk that existed when both
// files carried hand-copied duplicates. See project memory:
// project_chat_js_restructuring_plan.md.
//
// report-writers.mjs is an ES module file, and this file (chat.js) is plain
// CommonJS -- require() cannot load an ES module, so the functions are loaded
// with a dynamic import() instead, assigned once per cold start below. This is
// a standard, Netlify/Node-supported bridge (verified 2026-07-02), not a hack.
let getPrompt, extractWeeklyPlan, writeWeeklyPlan, extractQuarterly,
    writeQuarterlyReview, extractVTPlaybook, wrapVTPlaybookInTemplate;

const reportWritersReady = import('./lib/report-writers.mjs').then((mod) => {
  ({
    getPrompt,
    extractWeeklyPlan,
    writeWeeklyPlan,
...
// Virtual Team Room (2026-07-02): box allowlist, box labels, the VT output-override
// block, and the entire room='virtualteam' handler now live in
// netlify/functions/lib/virtual-team-room.js. Loaded via require() below (plain
// CommonJS, unlike report-writers.mjs — no ES-module bridge needed here). See
// project memory: project_chat_js_restructuring_plan.md.
const { handleVirtualTeamRoom } = require('./lib/virtual-team-room.js');

// Prep Room (2026-07-02): route detector, both output-override block builders,
// and the entire room='prep' handler now live in netlify/functions/lib/prep-room.js.
// A real production bug was found and fixed during this extraction — see that
// file's header comment for details (cs13Prompt/cs14Prompt/etc. were declared
// out of scope from where they were used, which would throw on a real quarterly
// background-build trigger). Loaded via require() below. See project memory:
// project_chat_js_restructuring_plan.md.
...
// detectPrepRoute, buildWeeklyPlanOutputBlock, buildQuarterlyOutputBlock, and the
// entire room='prep' handler now live in netlify/functions/lib/prep-room.js
// (required in above). Extracted 2026-07-02 — see project memory:
// project_chat_js_restructuring_plan.md.

// buildVTPlaybookOutputBlock, the VT box allowlist/labels, and the entire
// room='virtualteam' handler now live in netlify/functions/lib/virtual-team-room.js
// (required in below). Extracted 2026-07-02 — see project memory:
// project_chat_js_restructuring_plan.md.

// wrapVTPlaybookInTemplate now lives in netlify/functions/lib/report-writers.mts
// (required in below), shared with generate-report-background.mts. Extracted
...
// getPrompt now lives in netlify/functions/lib/report-writers.mts (required in below).
...
// extractWeeklyPlan, writeWeeklyPlan, extractQuarterly, extractVTPlaybook, and
// writeQuarterlyReview now live in netlify/functions/lib/report-writers.mts
// (required in below), shared with generate-report-background.mts. Extracted
// 2026-07-02 — see project memory: project_chat_js_restructuring_plan.md.
...
exports.handler = async (event) => {
  ...
  // Wait for the shared report-writer module (dynamic import, see top of file) —
  // resolves instantly after the first cold-start call, since Node caches the
  // module import after that.
  ...
  try {
    ...
    if (room === 'chat') {
      ...
      // netlify/functions/lib/virtual-team-room.js. Same behavior as the prior
      // inline block — box validation, VT output-override, background-build
      // trigger phrases, and the receipt close block. Extracted 2026-07-02.
      const vtResult = await handleVirtualTeamRoom(
        { boxId, messages, context, userName, userId },
        { getPrompt, buildContextString, buildReceiptCloseBlock, csReceiptShouldLoad, corsHeaders, SUPABASE_URL, SUPABASE_SERVICE_KEY }
      );
      if (vtResult.response) return vtResult.response;
      systemPrompt = vtResult.systemPrompt;

    } else if (room === 'prep') {
      // Full room='prep' handling now lives in netlify/functions/lib/prep-room.js.
      // Same behavior as the prior inline block, PLUS a real production bug fix
      // (a scoping bug that would have thrown on a real quarterly background-build
      // trigger) — see that file's header comment. Extracted 2026-07-02.
      const prepResult = await handlePrepRoom(
        { messages, context, userName, userId },
        { getPrompt, buildContextString, buildReceiptCloseBlock, csReceiptShouldLoad, corsHeaders, SUPABASE_URL, SUPABASE_SERVICE_KEY }
      );
      if (prepResult.response) return prepResult.response;
